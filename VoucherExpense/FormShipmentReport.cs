...
namespace VoucherExpense
{
    public partial class FormShipmentReport : Form
    {
        ...
        void Calculate(int monthFrom, int dayFrom, int monthTo, int dayTo, int id)
        {
            ...
            cmd.Connection = conn;
            cmd.CommandText = "  select p.Name 产品名, count(sd.ProductID) 次数,p.Price 单价,sum(sd.Volume) 总量,cast(sum(sd.Cost) as decimal(38,2))金额,cast(sum(sd.Volume)*p.EvaluatedCost as decimal(38,2)) as 估成本,cast(1-(sum(sd.Volume)*p.EvaluatedCost)/cast(sum(sd.Cost) as decimal(38,2)) as decimal(38,2))*100  as 毛利率 from Product p,Shipment s,ShipmentDetail sd where p.ProductID=sd.ProductID and s.ID=sd.ShipmentID and s.customer= " + id + " and s.ShipTime<='" + MyFunction.IntHeaderYear + monthTo.ToString("00") + dayTo.ToString("00") + "' and s.ShipTime>='" + MyFunction.IntHeaderYear + monthFrom.ToString("00") + dayFrom.ToString("00") + "' and sd.Volume>0 and sd.Cost>0 group by p.Name,p.Price,sd.Volume,p.EvaluatedCost" + "  select s.ShipCode 凭证号,s.ShipTime 出货时间,CONVERT(VARCHAR(15),CAST(CONVERT(DECIMAL(12,2),LTRIM(s.Cost)) AS MONEY),1) 金额 from Shipment s where s.customer= " + id + " and  s.ShipTime>='" + MyFunction.IntHeaderYear + monthFrom.ToString("00") + dayFrom.ToString("00") + "'  and s.ShipTime<='" + MyFunction.IntHeaderYear + monthTo.ToString("00") + dayTo.ToString("00") + "'";
